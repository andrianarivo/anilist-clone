  ...
  const pinchGesture = Gesture.Pinch()
    .onUpdate((event) => {
      ...
      if (savedScale.value > 0) {
        translateX.value = savedTranslateX.value + focalX * (scale.value / savedScale.value - 1);
        translateY.value = savedTranslateY.value + focalY * (scale.value / savedScale.value - 1);
      }

      // Clamp translation to boundaries
      const maxTranslateX = Math.max(0, (screenWidth * scale.value - screenWidth) / 2);
      const maxTranslateY = Math.max(0, (screenHeight * scale.value - screenHeight) / 2);
  ...
  const panGesture = Gesture.Pan()
    .onUpdate((event) => {
      ...
      if (scale.value > MIN_SCALE) {
        ...
        // Clamp translation to boundaries
        translateX.value = Math.max(-maxTranslateX, Math.min(newTranslateX, maxTranslateX));
        translateY.value = Math.max(-maxTranslateY, Math.min(newTranslateY, maxTranslateY));
      }
    })
    .onEnd(() => {
      savedTranslateX.value = translateX.value;
